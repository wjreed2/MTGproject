...
## DECIDED — Plan feature on by default (2026-09-24)
Saved Plan strategy/wincon identity is **on** unless Settings → Deck Plan is turned off (`mtg_deck_plan === '0'`). Off hides the Plan button and `getDeckPlan` clears identity on read while keeping Commander Gameplan numbers; stored `deck.plan` JSON is not wiped. The wizard reads and saves the stored plan, and a save while the feature is off does not erase identity. Architecture strategy/wincon identity lives on `architectureOverrides` with auto-defaults from semantic goals/themes.
...
## DECIDED — Game tracker seat order is user-set
Players can reorder seats in the New Game list (before start) and on an active game (desktop cards and tablet ⋯ menu). Array order is the table; whose turn it is follows the player, not the slot. First player is still rolled at start.

## DECIDED — Game tracker amounts use a number wheel
In-game X and log-event amounts are chosen on a scroll-snap wheel, not a typed number field.
