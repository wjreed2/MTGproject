...
**Plan feature (default off):** Saved Plan strategy/wincon identity is disabled unless Settings → **Deck Plan** is on (`localStorage.mtg_deck_plan === '1'`). `getDeckPlan` strips identity while keeping Commander Gameplan numbers. Architecture does **not** depend on Plan for bands — see identity below. Catalogs in `deck-plan.js` remain for labels and subsection pickers.
