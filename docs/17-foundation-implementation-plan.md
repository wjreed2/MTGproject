...
19. Calibration — **open**, config-only. Detector now uses CardIR + tags + oracle ([20](./20-foundation-calibration-infra.md)). Lab injects experimental config via `cloneFoundationConfig`. Next: inspect the 23 synthetics (Need → Mechanism → Evidence → Coverage) before changing numbers.  
