...
**By Architecture view (SHIPPED v1)** — [22-deck-architecture.md](./22-deck-architecture.md). Third list toggle. Visualization Foundation functions ≠ five-capability evaluator. Mana Sources = lands plus Ramp. User Set-primary (7C) writes Primary role and keeps extras. **Plan identity is off by default** (2026-09-18); Architecture bands come from goals / `architectureOverrides` — see [05-decisions.md](./05-decisions.md).
