...
const PROMPT_VERSION = 'p9'; // p9: synthesized axes are NEEDS-only; tribal.synergy provides "chosen type" vs null-changer split (p8: draw-vs-loot + conditional riders)
...
- param narrows an axis (tribal type like "Goblin", spell class, counter kind, card class like "creature"/"land"). Apply it on BOTH sides of the join, and only where the card itself is restricted:
  ...
  - \`body.legendary\` and \`ability.activated\` are NEEDS-only: write them when a card's engine keys on legendary bodies or on creatures' activated abilities (Thranduil). NEVER write provides on them — the engine derives those from the type line and faces itself, and an authored copy double-counts.
