...
| **24** | Suggested Adds Phase A0/A1 (raw badge + plan H + Why) | **Design** | Cuts/Adds 13 v2 Phase A | See `suggested-adds-improvement-plan.md` §5 A0–A1. Prereq for #25. |
| **25** | Suggested Adds Phase A1.5 — Plan envelope + semantics handoff | **Ready** | Cuts/Adds 13 v2 A1.5 | Plan parent + sub-tags; partner `engine2/` owns type inference. Canonical: plan §1.5, §13–§16. |
...
| Entry | Status | Note |
|-------|--------|------|
| Suggested Adds improvement + Entry 13 v2 | **Design drafted** | Canonical: [`suggested-adds-improvement-plan.md`](./suggested-adds-improvement-plan.md). **Prompt 24** = A0/A1 (raw badge + H). **Prompt 25** = A1.5 Plan envelope + semantics handoff. Phase B = wizard v2 / Cuts shielding. |

### Not in this doc yet (implement prompt text not pasted below)

| Entry | Status | Note |
|-------|--------|------|
| Phase A — plan term H + badge + role gate | Prompt **24** | After A0 sign-off. |
| Phase A1.5 — Plan envelope + semantics handoff | Prompt **25** | After #24 A0 or can parallel if no display-layer conflict; prefer after A0. |
...
# Prompt 24 of 25 — Suggested Adds Phase A (raw badge + plan H; no display/S)
...
**Related:** Prompt 25 (A1.5 Plan envelope) — prefer after A0 from this prompt; can share PR if scoped carefully.
