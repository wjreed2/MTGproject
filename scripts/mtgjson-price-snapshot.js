 ...
 *
 * Also exported as runSnapshot({ db?, url? }) so the in-process daily cron in
 * server.js can run it against the shared pool — idempotent per date.
 */
'use strict';
...
const DEFAULT_SRC = 'https://mtgjson.com/api/v5/AllPricesToday.json.gz';
...
/**
 * Fetch + upsert one day of MTGJSON prices.
 * @param {object} [opts]
 * @param {object} [opts.db]  existing mysql2 pool/connection (reused by the server cron)
 * @param {string} [opts.url] source URL (defaults to AllPricesToday)
 * @param {function} [opts.log] logger (defaults to console.log)
 * @returns {Promise<{date:string, rows:number}>}
 */
async function runSnapshot(opts = {}) {
  const url = opts.url || DEFAULT_SRC;
  const log = opts.log || console.log;
  log(`[price-snapshot] fetching ${url} …`);
  const res = await fetch(url, { headers: { 'User-Agent': 'MTGproject price-snapshot' } });
  if (!res.ok) throw new Error(`download failed: ${res.status} ${res.statusText}`);
  const gz = Buffer.from(await res.arrayBuffer());
  const json = (url.endsWith('.gz') ? zlib.gunzipSync(gz) : gz).toString('utf8');
  const parsed = JSON.parse(json);
  const date = parsed?.meta?.date;
  const data = parsed?.data || {};
  if (!date) throw new Error('no meta.date in MTGJSON payload');
  const uuids = Object.keys(data);
  log(`[price-snapshot] ${parsed.meta.version} — date ${date} — ${uuids.length.toLocaleString()} priced uuids`);
  ...
  log(`[price-snapshot] ${rows.length.toLocaleString()} rows with a paper price`);
  ...
  try {
    ...
    log(`[price-snapshot] done for ${date} (${rows.length.toLocaleString()} rows).`);
...
if (require.main === module) {
  ...
  const url = i !== -1 && process.argv[i + 1] ? process.argv[i + 1] : DEFAULT_SRC;
  runSnapshot({ url }).catch(e => { console.error('[price-snapshot] FAILED:', e.message); process.exit(1); });
}
