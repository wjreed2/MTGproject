...
function fmt(r) {
  ...
  return `[${when}] ${r.email || '?'} · deck ${r.deck_id || '?'} · ${r.engine}/${r.goal || '—'}\n  ${r.card_name}${score}: ${r.feedback}`;
}
      ...
      for (const r of rows) {
        ...
        await db.query(
          `INSERT IGNORE INTO suggestion_feedback
             (account_id, deck_id, engine, goal, card_name, score, feedback, created_at, source, remote_id)
           VALUES (0,?,?,?,?,?,?,?,'prod',?)`,
          [r.deck_id, r.engine, r.goal, r.card_name, r.score,
           `${r.feedback}\n[from: ${r.email || 'unknown'}]`, r.created_at, r.id]);
        console.log(fmt(r) + '\n');
        pulled++;
      }
